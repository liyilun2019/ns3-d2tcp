...
void dstSocketRecv (Ptr<Socket> socket)
{
  ...
  Address me,peer;
  socket->GetSockName(me);
  socket->GetPeerName(peer);
  InetSocketAddress add_me = InetSocketAddress::ConvertFrom(me);
  InetSocketAddress add_peer=InetSocketAddress::ConvertFrom(peer);
  NS_LOG_INFO (add_me.GetIpv4()<<" "<<add_peer.GetIpv4());
...
main (int argc, char *argv[])
{
  ...
  Ptr<Socket> dstSocket = Socket::CreateSocket (nDst, TypeId::LookupByName ("ns3::TcpSocketFactory"));
  uint16_t dstport = 12345;
  Ipv4Address dstaddr ("10.20.1.2");
  ...
  Simulator::Schedule (Seconds (1.0),&BindSock, srcSocket3, SrcToRtr2);
  Simulator::Schedule (Seconds (1.1),&StartFlow, srcSocket3, dstaddr, dstport);
  // Fourth again as normal (goes via Rtr1)
  Simulator::Schedule (Seconds (1.0),&BindSock, srcSocket4, Ptr<NetDevice>(0));
  Simulator::Schedule (Seconds (1.1),&StartFlow, srcSocket4, dstaddr, dstport);
...
void StartFlow (Ptr<Socket> localSocket,
                ...
                uint16_t servPort)
{
  NS_LOG_INFO ("Starting flow at time " <<  Simulator::Now ().GetSeconds ()+100);
  currentTxBytes = 0;
  localSocket->Bind ();
