...
NS_LOG_COMPONENT_DEFINE ("TimeAppChild");
...
NS_OBJECT_ENSURE_REGISTERED (TimeAppChild);


TimeAppChild::TimeAppChild ()
  : m_state (NOT_STARTED),
  m_initialSocket (0),
  m_txBuffer (Create<TimeAppChildTxBuffer> ()),
...
void
TimeAppChild::SetMtuSize (uint32_t mtuSize)
{
...
Ptr<Socket>
TimeAppChild::GetSocket () const
{
  return m_initialSocket;
}


TimeAppChild::State_t
TimeAppChild::GetState () const
{
  return m_state;
}
...
std::string
TimeAppChild::GetStateString () const
{
...
// static
std::string
TimeAppChild::GetStateString (TimeAppChild::State_t state)
{
  switch (state)
    {
    case NOT_STARTED:
  ...
  if (m_state == NOT_STARTED)
    {
      ...
      m_initialSocket->SetAcceptCallback (MakeCallback (&TimeAppChild::ConnectionRequestCallback,
                                                        this),
                                          MakeCallback (&TimeAppChild::NewConnectionCreatedCallback,
                                                        this));
      m_initialSocket->SetCloseCallbacks (MakeCallback (&TimeAppChild::NormalCloseCallback,
                                                        this),
                                          MakeCallback (&TimeAppChild::ErrorCloseCallback,
                                                        this));
      m_initialSocket->SetRecvCallback (MakeCallback (&TimeAppChild::ReceivedDataCallback,
                                                      this));
      m_initialSocket->SetSendCallback (MakeCallback (&TimeAppChild::SendCallback,
                                                      this));
...
bool
TimeAppChild::ConnectionRequestCallback (Ptr<Socket> socket,
...
void
TimeAppChild::NewConnectionCreatedCallback (Ptr<Socket> socket,
                                                  const Address &address)
{
  NS_LOG_FUNCTION (this << socket << address);

  socket->SetCloseCallbacks (MakeCallback (&TimeAppChild::NormalCloseCallback,
                                           this),
                             MakeCallback (&TimeAppChild::ErrorCloseCallback,
                                           this));
  socket->SetRecvCallback (MakeCallback (&TimeAppChild::ReceivedDataCallback,
                                         this));
  socket->SetSendCallback (MakeCallback (&TimeAppChild::SendCallback,
                                         this));
...
void
TimeAppChild::NormalCloseCallback (Ptr<Socket> socket)
{
...
void
TimeAppChild::ErrorCloseCallback (Ptr<Socket> socket)
{
  NS_LOG_FUNCTION (this << socket);
...
void
TimeAppChild::ReceivedDataCallback (Ptr<Socket> socket)
{
  NS_LOG_FUNCTION (this << socket);
  ...
  while ((packet = socket->RecvFrom (from)))
    {
      ...
      switch (httpHeader.GetContentType ())
        ...
        case ThreeGppHttpHeader::MAIN_OBJECT:
          ...
          m_txBuffer->RecordNextServe (socket,
                                       Simulator::Schedule (processingDelay,
                                                            &TimeAppChild::ServeNewMainObject,
                                                            this, socket),
        ...
        case ThreeGppHttpHeader::EMBEDDED_OBJECT:
          ...
          m_txBuffer->RecordNextServe (socket,
                                       Simulator::Schedule (processingDelay,
                                                            &TimeAppChild::ServeNewEmbeddedObject,
                                                            this, socket),
                                       httpHeader.GetClientTs ());
          break;
...
void
TimeAppChild::SendCallback (Ptr<Socket> socket, uint32_t availableBufferSize)
{
...
void
TimeAppChild::ServeNewMainObject (Ptr<Socket> socket)
{
  NS_LOG_FUNCTION (this << socket);
...
void
TimeAppChild::ServeNewEmbeddedObject (Ptr<Socket> socket)
{
  NS_LOG_FUNCTION (this << socket);
...
uint32_t
TimeAppChild::ServeFromTxBuffer (Ptr<Socket> socket)
{
  NS_LOG_FUNCTION (this << socket);
...
void
TimeAppChild::SwitchToState (TimeAppChild::State_t state)
{
  ...
  m_state = state;
  NS_LOG_INFO (this << " TimeAppChild " << oldState
                    << " --> " << newState << ".");
...
TimeAppChildTxBuffer::TimeAppChildTxBuffer ()
{
  NS_LOG_FUNCTION (this);
}

bool
TimeAppChildTxBuffer::IsSocketAvailable (Ptr<Socket> socket) const
{
...
void
TimeAppChildTxBuffer::AddSocket (Ptr<Socket> socket)
{
  NS_LOG_FUNCTION (this << socket);
...
void
TimeAppChildTxBuffer::RemoveSocket (Ptr<Socket> socket)
{
  NS_LOG_FUNCTION (this << socket);
...
void
TimeAppChildTxBuffer::CloseSocket (Ptr<Socket> socket)
{
  NS_LOG_FUNCTION (this << socket);
...
void
TimeAppChildTxBuffer::CloseAllSockets ()
{
  NS_LOG_FUNCTION (this);
...
bool
TimeAppChildTxBuffer::IsBufferEmpty (Ptr<Socket> socket) const
{
  std::map<Ptr<Socket>, TxBuffer_t>::const_iterator it;
  it = m_txBuffer.find (socket);
  NS_ASSERT_MSG (it != m_txBuffer.end (),
...
Time
TimeAppChildTxBuffer::GetClientTs (Ptr<Socket> socket) const
{
  std::map<Ptr<Socket>, TxBuffer_t>::const_iterator it;
  it = m_txBuffer.find (socket);
  NS_ASSERT_MSG (it != m_txBuffer.end (),
...
TimeAppChildTxBuffer::GetBufferContentType (Ptr<Socket> socket) const
{
  std::map<Ptr<Socket>, TxBuffer_t>::const_iterator it;
  it = m_txBuffer.find (socket);
  NS_ASSERT_MSG (it != m_txBuffer.end (),
...
uint32_t
TimeAppChildTxBuffer::GetBufferSize (Ptr<Socket> socket) const
{
  std::map<Ptr<Socket>, TxBuffer_t>::const_iterator it;
  it = m_txBuffer.find (socket);
  NS_ASSERT_MSG (it != m_txBuffer.end (),
...
bool
TimeAppChildTxBuffer::HasTxedPartOfObject (Ptr<Socket> socket) const
{
  std::map<Ptr<Socket>, TxBuffer_t>::const_iterator it;
  it = m_txBuffer.find (socket);
  NS_ASSERT_MSG (it != m_txBuffer.end (),
...
void
TimeAppChildTxBuffer::WriteNewObject (Ptr<Socket> socket,
...
void
TimeAppChildTxBuffer::RecordNextServe (Ptr<Socket>    socket,
...
void
TimeAppChildTxBuffer::DepleteBufferSize (Ptr<Socket> socket, uint32_t amount)
{
...
void
TimeAppChildTxBuffer::PrepareClose (Ptr<Socket> socket)
{
  NS_LOG_FUNCTION (this << socket);
  std::map<Ptr<Socket>, TxBuffer_t>::iterator it;
  it = m_txBuffer.find (socket);
