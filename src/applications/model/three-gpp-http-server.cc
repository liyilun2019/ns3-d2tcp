  ...
  while ((packet = socket->RecvFrom (from)))
    {
      ...
      Time deadline = httpHeader.GetDeadline();
      NS_LOG_INFO(this <<" from is : " << from
                       <<" , packet is : "<< packet
                       <<" , head is : " << httpHeader
                       <<" , socket is : "<< socket
                       <<" , deadline is : " << deadline
                       <<" , type is : "<< httpHeader.GetContentType());
...
ThreeGppHttpServer::ServeFromTxBuffer (Ptr<Socket> socket)
{
  ...
  NS_LOG_INFO (this << " Socket has " << socketSize
  ...
  uint32_t contentSize = std::min (txBufferSize, socketSize  - 22 - 8);
