...
ThreeGppHttpHeader::Print (std::ostream &os) const
{
  ...
  os << "(Content-Type: " << m_contentType
     ...
     << " deadline : " << TimeStep (m_deadline).As (Time::S)
     << ")";
}
...
void
ThreeGppHttpHeader::SetDeadline (Time deadline)
{
  NS_LOG_FUNCTION (this << deadline.As (Time::S));
  m_deadline = deadline.GetTimeStep ();
}


Time
ThreeGppHttpHeader::GetDeadline () const
{
  return TimeStep (m_deadline);
}
